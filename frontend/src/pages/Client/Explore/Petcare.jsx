...
const Petcare = () => {
  ...
  return (
    <div className={`bg-[#f7f9fb] text-[#191c1e] min-h-screen flex flex-col font-['Inter'] relative overflow-x-hidden ${isDrawerOpen ? 'overflow-hidden' : ''}`}>
      <style>{`
        .hero-bg {
          background-image: linear-gradient(rgba(0, 0, 0, 0.55), rgba(0, 0, 0, 0.55)), url(https://images.unsplash.com/photo-1450526859677-cde19c1a020e?auto=format&fit=crop&w=1350&q=80);
      ...
      <aside className={`fixed left-0 top-0 h-full w-full max-w-md bg-white z-[70] shadow-2xl flex flex-col border-r border-[#bccac0] transition-transform duration-300 ease-in-out ${isDrawerOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <div className="flex-grow overflow-y-auto p-6 space-y-[48px]">
          <div className="space-y-[4px]">
            ...
            <div className="text-[18px] text-[#006948] font-semibold">Specify the petcare request</div>
          </div>
          <div className="space-y-[4px]">
      ...
      <ClientHeader pageTitle="Petcare" />
      <main className="flex-grow pt-16">
        <section className="hero-bg py-[80px] px-[32px] text-center text-white mb-[48px] min-h-[400px] flex items-center justify-center">
          <div className="max-w-7xl mx-auto">
            <h1 className="text-[40px] font-bold mb-[12px] drop-shadow-lg leading-[1.2] tracking-[-0.02em]">Trusted care for your pets.</h1>
            <p className="text-[18px] max-w-2xl mx-auto drop-shadow-md opacity-95 leading-[1.6]">Book experienced pet sitters, walkers, and home care professionals who treat your animals like family.</p>
          </div>
        </section>
        ...
        <div className="max-w-7xl mx-auto px-[32px] w-full">
          ...
          <section className="mb-[80px]">
            <h2 className="text-[32px] font-semibold text-[#191c1e] mb-[24px] leading-[1.3] tracking-[-0.01em]">Featured Petcare Specialists</h2>
            <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-[24px]">
              {[{
                name: 'Kamala Nirosha', role: 'Senior Pet Nurse', rating: '5.0', reviews: 120, avail: 'Available'
              },{
                name: 'Vasantha Perera', role: 'Dog Walker', rating: '4.9', reviews: 88, avail: 'Available'
              },{
                name: 'Samanthi Jay', role: 'Pet Groomer', rating: '4.9', reviews: 76, avail: 'Available'
              },{
                name: 'Hiran Silva', role: 'Animal Care Specialist', rating: '4.8', reviews: 65, avail: 'Unavailable'
              },{
                name: 'Priya Kumar', role: 'Pet Sitter', rating: '4.8', reviews: 54, avail: 'Unavailable'
              }].map((p, idx) => (
                <div key={idx} className={`bg-white rounded-xl p-[24px] shadow-[0_4px_20px_-4px_rgba(6,78,59,0.05)] border-2 ${idx===0? 'border-[#006948]/20':'border-[#eceef0]'} flex flex-col items-center text-center relative overflow-hidden group hover:shadow-lg transition-all ${idx===0? 'scale-105 z-10':''}`}>
                  ...
                  <div className="w-24 h-24 rounded-full mb-[12px] overflow-hidden border-2 border-[#006948] p-1">
                    <img alt={p.name} className="w-full h-full object-cover rounded-full" src={`https://source.unsplash.com/collection/888146/200x200?sig=${idx}`} />
                  </div>
                  ...
                  <div className="flex items-center gap-[4px] mb-[24px]"><span className="material-symbols-outlined text-[#006948] text-[18px]" style={{ fontVariationSettings: '"FILL" 1' }}>star</span><span className="text-[14px] font-semibold text-[#191c1e]">{p.rating}</span><span className="text-[#3d4a42] text-sm">({p.reviews})</span></div>
                  <div className={`text-${idx===0? '#006948':'#006948'} text-[14px] font-semibold mb-[24px]`}>{p.avail}</div>
          ...
          <section className="mb-[80px]">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-[24px]">
              <h2 className="text-[32px] font-semibold text-[#191c1e]">Active Petcare Service Providers</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-[24px]">
              {[{name:'Kamala',role:'Pet Nurse',city:'Colombo',rating:'5.0',slots:'4/10'},{name:'Ravi',role:'Pet Sitter',city:'Kandy',rating:'4.9',slots:'6/10'},{name:'Maya',role:'Dog Walker',city:'Galle',rating:'4.8',slots:'3/10'},{name:'Sunil',role:'Groomer',city:'Negombo',rating:'4.7',slots:'5/10'},{name:'Anu',role:'Cat Sitter',city:'Matara',rating:'4.9',slots:'7/10'}].map((p, idx) => (
                <div key={idx} className="bg-white rounded-xl p-[24px] shadow-[0_4px_20px_-4px_rgba(6,78,59,0.05)] border border-[#eceef0] flex flex-col h-full hover:border-[#006948]/40 hover:shadow-lg transition-all group">
                  <div className="flex items-center gap-[24px] mb-[24px]">
                    <div className="w-16 h-16 rounded-full overflow-hidden border-2 border-[#006948]/10 flex-shrink-0">
                      <img alt={p.name} className="w-full h-full object-cover" src={`https://source.unsplash.com/collection/888146/100x100?sig=${idx+10}`} />
                    </div>
